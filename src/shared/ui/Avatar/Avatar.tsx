...
export const Avatar = memo(({ className, src, size, alt, online = false }: AvatarProps) => {
  const backgroundColor = useMemo(() => {
    ...
    return altToLower < 'd' ? 'green' : altToLower < 'h' ? 'cyan' : altToLower < 'l' ? 'indigo' : altToLower < 'p' ? 'amber' : altToLower < 't' ? 'purple' : 'pink'
  }, [alt])
  ...
  const fontSize = useMemo(() => size <= 24 ? 10 : size <= 32 ? 12 : size <= 48 ? 16 : size <= 64 ? 20 : size <= 96 ? 30 : 44, [size])
